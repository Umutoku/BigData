...
object Joins extends App {
  ...
  val guitarsDF = spark.read
    ...
    .json("spark-essentials/src/main/resources/data/guitars.json")

  val guitaristsDF = spark.read
    .option("inferSchema", "true")
    .json("spark-essentials/src/main/resources/data/guitarPlayers.json")

  val bandsDF = spark.read
    .option("inferSchema", "true")
    .json("spark-essentials/src/main/resources/data/bands.json")
  ...
  //val joinCondition = guitaristsDF.col("band") === bandsDF.col("id")
  // 1
  val maxSalariesPerEmpDF = salariesDF.groupBy("emp_no").agg(max("salary").as("maxSalary"))
  val employeesSalariesDF = employeesDF.join(maxSalariesPerEmpDF,"emp_no")
//    val maxSalariesDF = employeesDF
//    .join(salariesDF,employeesDF.col("emp_no") === salariesDF.col("emp_no"))
//  maxSalariesDF
...
//    .show()
 // employeesSalariesDF.show()
  ...
  val employeesManageriesDF = employeesDF.join(deptManagersDF,employeesDF.col("emp_no") === deptManagersDF.col("emp_no"),"left_anti")

  //employeesManageriesDF.show()
