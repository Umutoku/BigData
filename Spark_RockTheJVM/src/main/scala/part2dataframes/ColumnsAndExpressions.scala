...
object ColumnsAndExpressions extends App {
  ...
  val moreCarsDF = spark.read.option("inferSchema", "true").json("spark-essentials/src/main/resources/data/more_cars.json")
  ...
  /**
    ...
    */
    ...
    val movieDF = spark.read.option("inferSchema","true").json("spark-essentials/src/main/resources/data/movies.json")

    //movieDF.selectExpr("Creative_Type","Director").show()
    //movieDF.select($"Creative_Type", 'Director).show

    //movieDF.filter('Major_Genre === "Comedy" and 'IMDB_Rating > 6).show()

    movieDF.withColumn("Total_Profit",'US_Gross + 'Worldwide_Gross).show()
